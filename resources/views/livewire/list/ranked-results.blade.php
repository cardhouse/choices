{{--
    =====================================
    Ranked Results Table (Mobile Responsive)
    =====================================
...
<div>
    ...
    <div class="mb-8 sm:mb-12">
        <h1 class="text-2xl sm:text-4xl font-bold text-gray-900">Results</h1>
        <p class="text-base sm:text-lg text-gray-500 mt-1">See how your items performed in head-to-head voting.</p>
    </div>
            ...
            <table class="min-w-full text-xs sm:text-sm text-left">
                <thead class="text-gray-500 uppercase tracking-wider bg-gray-50">
                    <tr>
                        ...
                        <th class="px-2 sm:px-6 py-3 text-center">Wins</th>
                        @if($showVoteCounts)
                            <th class="px-2 sm:px-6 py-3 text-center">Votes</th>
                        @endif
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @foreach($results as $result)
                        <tr class="hover:bg-gray-50">
                            ...
                            @if($showVoteCounts)
                                <td class="px-2 sm:px-6 py-4 text-center text-gray-800">{{ $result['item']->votes_count ?? 0 }}</td>
                            @endif
                        </tr>
                    @endforeach
                </tbody>
